...
def test_single_comment_tokens(source, comment_text):
    token = next(
        tok for tok in tokens_without_whitespace(source) if tok[0] in (Comment, Error)
    )
...
def test_string_literals(source, expected):
    token = next(
        tok for tok in tokens_without_whitespace(source) if tok[0] in (String, Error)
    )
...
def test_character_literals(source, expected):
    token = next(
        tok
        for tok in tokens_without_whitespace(source)
